...
func (a *Adaptor) GetRequestURL(info *relaycommon.RelayInfo) (string, error) {
	...
	case common.ChannelTypeMaxkb:
		url := info.BaseUrl
		// 从 info.ChannelSetting 中获取 maxkbConfig
		        ...
		        if modelConfig, ok := configMap[info.UpstreamModelName]; ok {
		            if appKeyMap, ok := modelConfig.(map[string]interface{}); ok {
		                ...
		                for app, _ := range appKeyMap {
		                    url = fmt.Sprintf("%s/api/application/%s/chat/completions", url, app)
		                    break
		                }
		            }
		        }
...
func (a *Adaptor) SetupRequestHeader(c *gin.Context, header *http.Header, info *relaycommon.RelayInfo) error {
	...
	if info.ChannelType == common.ChannelTypeAzure {
		header.Set("api-key", maxkbApiKey)
		return nil
	}
	...
	if info.RelayMode == constant.RelayModeRealtime {
		...
		if swp != "" {
			items := []string{
				"realtime",
				"openai-insecure-api-key." + maxkbApiKey,
			...
			header.Set("Authorization", "Bearer "+maxkbApiKey)
		}
	} else {
		header.Set("Authorization", "Bearer "+maxkbApiKey)
	}
